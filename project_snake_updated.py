...
def draw_magic_circles():
    ...
    glColor3f(01.0, 1.0, 0)  # Blue color for magic circles
    for circle in magic_circles:
        x, y, _ = circle
        draw_circle(x, y, snake_size // 2 +3)

# Handle collosion with obstacle 
def handle_collision():
    global snake, score

    if check_collision_with_obstacles():
        # Implement your collision response here
        # For example, reset the game or reduce the snake's length
        snake = [[window_width // 2, window_height // 2]]  # Reset snake position
        score = 0  # Reset score

        # Additionally, you can reset other elements like magic circles or food
        # Resetting food position
        respawn_food()
        # Clearing magic circles
...
glutCreateWindow(b"MR. SILLY SNAKE")
